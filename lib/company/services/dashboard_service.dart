  ...
  Future<DashboardData> fetchDashboardData() async {
    ...
    try {
      ...
      for (var e in coreList) {
        ...
        if (id1 != null && id1.isNotEmpty && !combined.any((item) => (item['id']?.toString() ?? item['_id']?.toString()) == id1)) {
          combined.add(e);
        }
      }
    ...
    double productivity = totalEmployees > 0 ? (presentToday / totalEmployees) * 100 : 0.0;
    ...
    return DashboardData(
      ...
      efficiencyChange: (random.nextDouble() * 5.0) * (random.nextBool() ? 1 : -1),
